    ...
    {
        public async static Task<TResource?> Load<TResource>(string path, Delegates.ReportProgress? reportProgress = null)
        ...
        private static ChannelReader<ResourceLoadingProgress<T>> LoadInBackground<T>(string path) where T : Resource
        {
            ...
            Task.Run(async () =>
            {
                ...
                do
                {
                    ...
                    await result.Writer.WriteAsync(new ResourceLoadingProgress<T>(path, interactiveLoader.GetStage() /
