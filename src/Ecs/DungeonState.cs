...
public partial class DungeonState : GameState {
   ...
   void CreateSystems(GameStateController gameStates)
   {
	   ...
	   OnTurnEndSystems.Add(new OpenContainersSystem())
	   ...
	   turnHandlerSystem.ExecutePlayerTurn += () => { OnPlayerTurnSystems.Run(); };
	   turnHandlerSystem.ExecuteNpcTurn += () => { OnNpcTurnSystems.Run(); };
	   turnHandlerSystem.ExecuteTurnEnd += () => { OnTurnEndSystems.Run(); };
