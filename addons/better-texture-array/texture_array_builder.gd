...
func get_importer_name():
	return "texture_3d_builder" if is_3d else "texture_array_builder"

func get_visible_name():
	return "Texture3DBuilder" if is_3d else "TextureArrayBuilder"

func get_recognized_extensions():
	return ["ta-builder", "t3d-builder"]

func get_save_extension():
	return "tex3d" if is_3d else "texarr"

func get_resource_type():
	return "Texture3D" if is_3d else "TextureArray"
...
func _save_tex(images: Array, path: String, compression: int = Compress.UNCOMPRESSED, vram_compression: int = Image.COMPRESS_S3TC, flags: int = 7):
	...
	file.store_8(ord('G'))
	file.store_8(ord('D'))
	file.store_8(ord('3' if is_3d else 'A')) # Godot Texture3D or TextureArray
	file.store_8(ord('T')) # Godot streamable texture
	
	file.store_32(images[0].get_width() if num_images > 0 else 0)
	file.store_32(images[0].get_height() if num_images > 0 else 0)
