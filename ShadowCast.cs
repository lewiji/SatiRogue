...
public static class ShadowCast {
    ...
    private static void CastLight(MapData grid, Vector3i gridPosn, float viewRadius,
        ...
        int viewCeiling = (int)System.Math.Ceiling(viewRadius);
        ...
        for (int currentCol = startColumn; currentCol <= viewCeiling; currentCol++) {
            ...
            for (int yc = currentCol; yc >= 0; yc--) {
                ...
                int gridY = gridPosn.z + xc * txfrm.Yx + yc * txfrm.Yy;
                ...
                if (distanceSquared <= viewRadiusSq) {
                    grid.SetLight(gridVec , distanceSquared);
                }

                bool curBlocked = grid.IsWall(gridVec);
