...
public static class DictionaryX {
   ...
   /// contents.</returns>
   public static Dictionary<TKey, TValue> ToDotNet<TKey, TValue>(this Godot.Collections.Dictionary<TKey, TValue> dictionary)
      where TKey : notnull {
      ...
      for (var i = 0; i < keys.Count; i++) {
         ...
         if (key is null) {
            throw new InvalidOperationException(
               $"Dictionary contains a key of type {key?.GetType()} " + $"but expected type {typeof(TKey)}");
         }
         dynamic? value = values[i];

         if (value is not TValue) {
            throw new InvalidOperationException($"Dictionary contains a value of type {value?.GetType()} "
