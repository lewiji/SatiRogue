...
using Godot;
using GoDotLog;
using SatiRogue.lib.go_dot_net.src.provider;
...
public static class NodeX {
   ...
   public static T Autoload<T>(this Node node) where T : Node {
      var autoload = node.TryAutoload<T>();
   ...
   public static void Unawaited<T>(this Node _, ILog log, Func<T> callback) where T : Task {
      callback()
         .ContinueWith(task => {
