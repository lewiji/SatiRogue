...
public class InvalidStateTransition<TState> : Exception {
   ...
   /// <returns></returns>
   public InvalidStateTransition(TState current, TState desired) : base($"Invalid state transition between ${current} and ${desired}.") {
      Current = current;
      Desired = desired;
   }
}
...
public interface IMachineState<TState> {
   ...
   public virtual bool CanTransitionTo(TState state) {
      return true;
   }
}
...
public sealed class Machine<TState> where TState : IMachineState<TState> {
   ...
   Queue<TState> _pendingTransitions { get; set; } = new();
