...
public static class IProviderNodeX {
   ...
   static readonly ConditionalWeakTable<IProviderNode, ProviderState> _providerStates = new();

   static ProviderState GetState(IProviderNode node) {
      return _providerStates.GetOrCreateValue(node);
   }

   static void SetState(IProviderNode node, ProviderState state) {
   ...
   public static bool HasProvided(this IProviderNode provider) {
      return GetState(provider).HasProvided;
   }
   ...
   public static void Listen(this IProviderNode provider, Action<IProviderNode>? onProvided) {
   ...
   /// <param name="onProvided">Action to perform when the receiver has
   /// finished providing values.</param>
   public static void StopListening(this IProviderNode provider, Action<IProviderNode>? onProvided) {
      GetState(provider).OnProvided -= onProvided;
   }
}
