...
public static class DependentX {
   static readonly ConditionalWeakTable<IDependent, Dependencies> _deps = new();

   static Dependencies GetState(IDependent dependent) {
      return _deps.GetOrCreateValue(dependent);
   }

   static void SetState(IDependent dependent, Dependencies deps) {
      _deps.AddOrUpdate(dependent, deps);
   }

   static void SetDeps(this IDependent dependent, Dependencies deps) {
      ...
      if (dependent is Node node) {
         ...
         while (currentType != null) {
            if (currentType.GetInterface(nameof(IDependent)) != null) {
               ...
               var typeProps = currentType.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
         ...
         if (properties.Count < 1) {
            throw new Exception($"{currentType?.Name} does not have any dependencies. Make "
                                + "sure dependencies use the [Dependency] attribute and invoke " + "this.DependOn<Value>().");
         }

         var classType = typeof(DependentX);
         var getProviderMethod = classType.GetMethod(nameof(GetProvider), BindingFlags.Static | BindingFlags.NonPublic);
   ...
   static IProvider<T> GetProvider<T>(Node node) {
      ...
      throw new Exception("Nodes using dependencies need to implement Dependent.");
   }
}