...
public class DebugDrawDemoSceneCS : Spatial {
   ...
   public override void _Process(float delta) {
      ...
      var g = _create_graph("FPS", true, false, Engine.EditorHint ? DebugDrawCS.BlockPosition_LeftTop : DebugDrawCS.BlockPosition_RightTop,
         DebugDrawCS.GraphTextFlags_Current | DebugDrawCS.GraphTextFlags_Avarage | DebugDrawCS.GraphTextFlags_Max
         | DebugDrawCS.GraphTextFlags_Min, new Vector2(200, 80), custom_font);
      ...
      DebugDrawCS.DrawCylinder(
         new Transform(Basis.Identity, GetNode<Spatial>("Cylinder2").GlobalTransform.origin).Scaled(new Vector3(1, 2, 1)), Colors.Red);
      ...
      DebugDrawCS.DrawBoxXf(
         new Transform(new Basis(Vector3.Up, Mathf.Pi * 0.25f).Scaled(Vector3.One * 2), GetNode<Spatial>("Box3").GlobalTransform.origin),
         Colors.RosyBrown);
      ...
      DebugDrawCS.DrawRay(GetNode<Spatial>("Lines/3").GlobalTransform.origin,
         (target.GlobalTransform.origin - GetNode<Spatial>("Lines/3").GlobalTransform.origin).Normalized(), 3, Colors.Crimson);
      ...
      DebugDrawCS.DrawArrowRay(GetNode<Spatial>("Lines/4").GlobalTransform.origin,
         (target.GlobalTransform.origin - GetNode<Spatial>("Lines/4").GlobalTransform.origin).Normalized(), 8, Colors.Lavender, 0.5f, true);

      DebugDrawCS.DrawLineHitOffset(GetNode<Spatial>("Lines/5").GlobalTransform.origin, target.GlobalTransform.origin, true,
         Mathf.Abs(Mathf.Sin(OS.GetTicksMsec() / 1000.0f)), 0.25f, Colors.Aqua);
      ...
      // Lag Test
      GetNode<Spatial>("LagTest").Translation
         = ((Transform) GetNode<AnimationPlayer>("LagTest/RESET").GetAnimation("RESET").TrackGetKeyValue(0, 0)).origin
           + new Vector3(Mathf.Sin(OS.GetTicksMsec() / 100.0f) * 2.5f, 0, 0);
   ...
   void _more_tests() {
      ...
      foreach (var _ray in GetNode("HitTest/RayEmitter").GetChildren()) {
         if (_ray is RayCast ray) {
            DebugDrawCS.DrawLineHit(ray.GlobalTransform.origin, ray.GlobalTransform.Translated(ray.CastTo).origin, ray.GetCollisionPoint(),
               ray.IsColliding(), 0.15f);
         }
      }

      // Delayed line render
      DebugDrawCS.DrawLine(GetNode<Spatial>("LagTest").GlobalTransform.origin + Vector3.Up,
         GetNode<Spatial>("LagTest").GlobalTransform.origin + new Vector3(0, 3, Mathf.Sin(OS.GetTicksMsec() / 50.0f)), null, 0.5f);
   }
   ...
   void _graph_test() {
      ...
      _create_graph("randf", false, true, DebugDrawCS.BlockPosition_RightBottom, DebugDrawCS.GraphTextFlags_Avarage, new Vector2(256, 60),
         custom_font);
   ...
   DebugDrawCS.GraphParameters _create_graph(string title,
      bool is_fps,
      bool show_title,
      int pos,
      int flags,
